...
class PuppeteerHar {
     ...
     * @return {Promise<void>}
     */
    async start({ path, saveResponse, captureMimeTypes, debug } = {}) {
                ...
                if (this.saveResponse && method == 'Network.responseReceived') {
                    ...
                    if (response.status !== 204 &&
                        ...
                        const promise = this.client.send(
                            ...
                            params.body = responseBody.body;
                            params.base64Encoded = responseBody.base64Encoded;
                        }, (reason) => {
    ...
    async stop() {
        ...
        const har = fromLog(
            ...
            {content:this.saveResponse}
        );
        this.cleanUp();
        if (this.path) {
          const content = this.debug ? JSON.stringify(har, null, 1) : JSON.stringify(har);
            await promisify(fs.writeFile)(this.path, content);
        } else {
            return har;
        }
    }
